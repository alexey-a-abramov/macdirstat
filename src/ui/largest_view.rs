...
use crate::model::tree::{DirSummary, ExtDirSummary, FileTree, TreePath};
...
/// Render the folders containing a chosen extension, ranked by that extension's
/// bytes. Single-click selects, double-click navigates (via `activated`).
pub fn show_ext_folders(
    ui: &mut egui::Ui,
    tree: &FileTree,
    dirs: &[ExtDirSummary],
    selected: &mut Option<TreePath>,
    ...
    if dirs.is_empty() {
        ui.weak("No folders contain this type.");
        return;
    }

    let max = dirs.iter().map(|d| d.ext_bytes).max().unwrap_or(1).max(1);
    let bar_base = Color32::from_rgb(56, 132, 244);
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            for (rank, d) in dirs.iter().enumerate() {
                ...
                let frac = (d.ext_bytes as f32 / max as f32).clamp(0.0, 1.0);
                let bar_rect = Rect::from_min_size(
                    pos2(rect.left() + 8.0, rect.bottom() - 11.0),
                    vec2((rect.width() - 16.0) * frac, 6.0),
                );
                p.rect_filled(
                    bar_rect,
                    2.0,
                    if is_sel { Color32::WHITE } else { bar_base },
                );
