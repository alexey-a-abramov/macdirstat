...
- `FileTree` carries `root`, `root_path: String`, `extensions: Vec<ExtStat>`, `permission_denied: u64`, and `size_approximate: bool`. `size_approximate` is set by `scan` when the totals are lower bounds — a small-file floor (`min_file_size_bytes > 0`), a skipped excluded/cloud folder (`ScanCtx::excluded_hit`), or an unreadable directory (`permission_denied > 0`). It persists through the cache and `refresh_exists`. **Adding a field changes the postcard layout → bump `cache::CACHE_VERSION`** (now `2`) so stale caches are ignored.
...
- The currently `selected` node's rect is outlined (dark halo + white line) whenever it falls inside the displayed `view_root`, so a selection made in the tree is visible in the rectangle view.

### Tree view (`src/ui/tree_view.rs`)
- Collapsible rows with hand-drawn folder icons, zebra striping, right-aligned size column, and alpha-faded long filenames. Caps at `MAX_RENDERED_ITEMS = 2000` with an "… and N more" marker. Up/Down arrows navigate the flattened visible rows.
- **Tree ↔ treemap sync:** `show` takes `current_dir` (the navigated/treemap root) and `approximate`. Two one-shot triggers (tracked via `egui` temp data keyed `tree_last_expanded` / `tree_last_nav`): a **selection** change expands the selected node's ancestors; a **navigation** change (`current_dir`) expands ancestors *and* the folder itself (`expand_including_path`) so the same level the treemap shows is visible here. The changed row is then `scroll_to_rect(…, None)`'d into view (selection wins over nav). All gated on change so manual collapse/scroll isn't fought.
- **Approximate sizes:** when `approximate` (set by `FileTree::size_approximate` ‖ a stopped scan), directory size labels are prefixed `~` (lower bound) — file sizes stay exact — and a small "≈ sizes are approximate" caption with an explanatory tooltip appears under the branding.

### Settings (`src/settings.rs`)
- Fields: `ignore_cloud_storage`, `skip_duplicate_inodes`, `optimization_mode`, `min_file_size_mb`, `scan_threads` (Advanced: 0 = auto/one-per-core; >0 runs the scan in a dedicated rayon pool — `FileTree::scan` then broadcasts on *that* pool to drain its workers' ext maps), and `custom_excludes: Vec<String>` (user-defined folders to skip, persisted as repeated `exclude=<path>` lines). **No serde/toml** — a flat `key=value` text file; `serialize()`/`parse()` round-trip is unit-tested. `load()` is infallible (falls back to `Default`); `save()` swallows all I/O errors. `excluded_paths()` returns the built-in cloud roots (`~/Library/CloudStorage`, `~/Library/Mobile Documents`, `~/Dropbox`, `~/OneDrive`, `~/Google Drive`) when `ignore_cloud_storage`, plus every `custom_excludes` path. **Why cloud-skip matters:** cloud roots are File Provider mounts on the *same* volume (so device tricks don't catch them) that enumerate over the network — slow/hangy — and report dataless placeholders at full *logical* size, so scanning them both stalls the scan and inflates the reported total (~2× on a real `~/Library`).
